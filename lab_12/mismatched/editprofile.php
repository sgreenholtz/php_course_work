<?php
    ...
    if (isset($_POST['submit']))
    {
        ...
        if (!empty($new_picture))
        {
            ...
            else
            {
                ...
                echo '<p class="error">Your picture must be a GIF, JPEG, or ' .
                    'PNG image file no greater than ' . (MM_MAXFILESIZE / 1024) .
                    ' KB and ' . MM_MAXIMGWIDTH . 'x' . MM_MAXIMGHEIGHT .
                    ' pixels in size.</p>';
            }
        }
        ...
        if (!$error)
        {
            ...
            !empty($birthdate) && !empty($city) && !empty($state))
            {
                if (!empty($new_picture))
                {
                    $query = "UPDATE mismatch_user SET first_name = " .
                        "'$first_name', last_name = '$last_name', gender = " .
                        "'$gender', birthdate = '$birthdate', city = '$city'," .
                        " state = '$state', picture = '$new_picture' WHERE " .
                ...
                else
                {
                    $query = "UPDATE mismatch_user SET first_name = " .
                        "'$first_name', last_name = '$last_name', gender = " .
                        "'$gender', birthdate = '$birthdate', city = '$city'," .
                        " state = '$state' WHERE user_id = '" .
                        $_SESSION['user_id'] . "'";
                }
                mysqli_query($dbc, $query);

                echo '<p>Your profile has been successfully updated. Would ' .
                    'you like to <a href="viewprofile.php">view your profile' .
                    '</a>?</p>';
            ...
            {
                echo '<p class="error">You must enter all of the profile data ' .
                    '(the picture is optional).</p>';
            }
        }
    }
    ...
    {
        $query = "SELECT first_name, last_name, gender, birthdate, city, " .
            "state, picture FROM mismatch_user WHERE user_id = '" .
            $_SESSION['user_id'] . "'";
    ...
    <form enctype="multipart/form-data" method="post"
        ...
        <fieldset>
            ...
            <select id="gender" name="gender">
                ...
                <option value="F" <?php if (!empty($gender) && $gender == 'F')
            ...
            <input type="text" id="city" name="city"
                value="<?php if (!empty($city)) echo $city; ?>" /><br />

            <label for="state">State:</label>
            <input type="text" id="state" name="state"
                value="<?php if (!empty($state)) echo $state; ?>" /><br />
