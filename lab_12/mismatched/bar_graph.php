...
function draw_graph($category_array, $width, $height, $max_value, $filename)
{
    ...
    $bar_color = imagecolorallocate($img, 192, 192, 192);
    $border_color = imagecolorallocate($img, 64, 64, 64);
    ...
    for ($i = 0; $i < count($category_array); $i++)
    {
        imagefilledrectangle($img, ($i * $bar_width * 2) + $bar_width, $height,
            ...
            $height - (($height / $max_value) * $category_array[$i][1]),
            $bar_color);
        imagestringup($img, 5, ($i * $bar_width * 2) + ($bar_width),
            $height - 5, $category_array[$i][0], $text_color);
    }
